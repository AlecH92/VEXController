...
public class JoystickView extends View implements Runnable {
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			...
			if (onJoystickMoveListener != null)
				onJoystickMoveListener.onValueChanged(getAngle(), getPower(),
						getDirection(), getXvalue(), getYvalue());
		}
		if (onJoystickMoveListener != null
	...
	private int getXvalue() {
        int returnValue = map((xPosition - centerX),-joystickRadius,joystickRadius,0,200);
		return returnValue;
	}

	private int getYvalue() {
        int returnValue = map((yPosition - centerY),-joystickRadius,joystickRadius,0,200);
        return returnValue;
	}
				...
				public void run() {
					if (onJoystickMoveListener != null)
						onJoystickMoveListener.onValueChanged(getAngle(),
								getPower(), getDirection(), getXvalue(), getYvalue());
				}
			});
			try {
